...
2. Enable search only after server-side Exa configuration. Run a real user-approved research question rather than a provider liveness test.
...
- Exa is chosen as the first production search adapter for semantic research quality and a stable API. The interface remains replaceable, but no unreliable scraping of public search result pages is used as a hidden fallback.
