...
- The first production search adapter is Exa, selected through server-only configuration. Lack of a search configuration degrades `auto` to site/direct behavior and returns an explicit unavailable state for forced `web` mode.
