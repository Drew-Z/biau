...
- `dusk | garden | stellar` 不能只是同一界面的轻微换色；每个场景同时改变 Flow 调色板、全屏纹理/构图、面板与卡片材质及控制强调行为，并在浅色、深色下保持各自可辨识。
- 场景必须贯穿整个公共页面而不是止于 Hero：Flow 之外还要有整页静态底色、纹理和地平线/星图层，页脚继续使用同一场景材质，不能在首屏之后退回统一黑色或灰白底。
- 三个场景还必须拥有不同的动态语言：`dusk` 使用光谱潮线与颗粒错速漂移，`garden` 使用等高线呼吸与有机纹理游移，`stellar` 使用星点闪烁、星图巡航与局部边缘追光；不能用同一套位移动画只替换颜色。
- 动态层复用现有 Flow Canvas、场景 DOM 和项目板指针处理，不增加第二个 Canvas、第三方动画库或常驻 React 帧更新。指针视差只在精细指针且非 reduced-motion 时启用；启动遮罩、页面隐藏和 reduced-motion 必须暂停或静止。
- 桌面首页使用明确双栏，左侧介绍与右侧项目板的视觉中心垂直对齐，左侧正文仍保持左对齐；`1024px` 及以下恢复单栏阅读顺序。
...
- [x] 首页六种外观组合在 320、390、430 和桌面视口下保持可读、无溢出，真实 Logo 可见，场景/明暗偏好在刷新后保持，自动模式响应系统浅深色变化；三场景在 Flow、整页基底、纹理、面板和页脚材质上存在结构性差异，桌面双栏视觉中心平衡。
- [x] 三场景在正常动态模式下拥有可区分的整页动画签名；精细指针可驱动场景视差和项目板局部光照，粗指针保持纯环境动画；启动遮罩、页面隐藏和 reduced-motion 下无持续场景运动，恢复后不产生重复循环或布局偏移。
