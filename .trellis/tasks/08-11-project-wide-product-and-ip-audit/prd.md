...
- [ ] 桌面与手机视口均能完成新建会话、发送、等待、回答、引用展开、编辑重发、重试、历史恢复和全屏退出。
- [ ] API 冷启动和网络失败有阶段性状态、合理等待与可执行恢复动作，不会迅速伪装成低质量回答。
- [ ] 模型不可用、RAG 无命中和全网搜索不可用分别使用准确状态，不泄漏 provider、endpoint、token 或内部诊断。
- [ ] session、message、branch 与 citation 在刷新后保持一致；失败重试不会重复消息或污染历史。
- [ ] 至少一次经批准的真实业务请求完成模型回答、引用、持久化、失败恢复和移动端人工观察，并留下低敏验收记录。

### AI Daily

- [ ] Studio 编辑者能区分发现、证据不足、生成、待审、驳回、可发布、已发布和失败状态，并知道下一步操作。
- [ ] 公开 Feed 与详情页能处理 loading、fresh、stale、empty、404、410 和 network error，并提供符合状态的恢复动作。
- [ ] 真实来源保留标题、URL、发布时间/抓取时间和引用证据；去重、时效和排序结果可审核。
- [ ] 至少一期真实 Edition 完成来源采集、生成、人工审核、Publish Export、部署和公开 Feed/详情页桌面及手机验收。
- [ ] Cron 保持关闭时，页面和文档不会宣称“每日自动发布”。

### Project and naming system

- [ ] 所有公开项目获得独立的 maturity、availability、access、owner 与 CTA 结论，状态含义不混用。
- [ ] 不可用/未验证入口在 Hero、项目详情、视觉 source link、正文和助手知识中的直接 CTA 均被关闭，但案例内容继续可读。
- [ ] 画帆 / BIAU Canvas 以 planned 工具条目出现，不展示虚构 URL、截图、在线徽标或直接体验按钮。
- [ ] 已确认名称从单一注册表投影到公开页面、metadata、状态标签与助手知识；稳定技术标识保持不变。
- [ ] reference-only/internal 目录不会进入公开项目目录或知识索引。
- [ ] lint、build、相关 contract/check、UI smoke 和静态链接检查通过，且不覆盖用户已有状态快照。
- [ ] 首页六种外观组合在 320、390、430 和桌面视口下保持可读、无溢出，真实 Logo 可见，场景/明暗偏好在刷新后保持，自动模式响应系统浅深色变化。
