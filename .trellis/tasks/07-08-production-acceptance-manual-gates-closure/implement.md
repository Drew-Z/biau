...
- Xunqiu showcase follow-up: pushed `fix: replace private backend repo links` to the showcase repository so its own buttons point at public backend validation docs instead of the visitor-invisible GitHub repo. A live HTML check immediately after push still showed the old GitHub link on the public domain, so Cloudflare Pages may still need a redeploy or more time to pick up the new commit.
