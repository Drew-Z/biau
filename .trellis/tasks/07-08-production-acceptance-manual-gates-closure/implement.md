...
- AI Daily public assistant empty-state follow-up completed: public assistant knowledge now has a dedicated `site:ai-daily` item explaining that AI Daily can be visible as a column while first public posts still wait behind Studio-first source/issue/draft review, Publish Export, static export, Git diff review, and blog quality gates. AI Daily queries now classify as `blog-knowledge` before generic manual-queue wording, and the frontend, Express server, and Cloudflare Function local retrieval/fallback paths keep the same `site:ai-daily` scoring and answer body. Local eval now covers "AI 日报为什么没有公开文章/下一步怎么发布" and "博客里的 AI 日报在哪里看", with `assistant:eval` passing at `modelCalls=0`.
- Resource sharing public assistant empty-state follow-up completed: public assistant knowledge now has a dedicated `site:resources` item explaining that `资源分享 / Resources` is an artificial curation column, not an automatic bulk link-list generator. Resource-sharing queries now cite `site:resources` first and answer with Studio draft, human review, Publish Export, static export, Git diff, and blog-quality gates. The generic blog intent no longer gives AI Daily column status a blanket boost, so resources, AI Daily, and general knowledge-note questions stay separated across frontend fallback, Express server retrieval, and Cloudflare Function local retrieval. Local eval now covers resource-sharing empty state, column location, and no-auto-link-list questions, with `assistant:eval` passing at `modelCalls=0`.
- SEO build-warning cleanup completed: `SeoManager` now uses static public data imports for project and public blog summaries instead of dynamic imports that were already neutralized by other static imports in the main bundle. `npm.cmd run lint` passed, and `npm.cmd run build` passed without the previous `INEFFECTIVE_DYNAMIC_IMPORT` warnings for `src/data/blogCuration.ts` or `src/data/portfolio.ts`.
...
- Internal assistant durable-memory production acceptance completed for migration availability, explicit-consent save, duplicate suppression, archive, and restore. Read-only `/me` and memory-list requests returned `200`; the accepted memory remained `ACTIVE` after restore. No token, member fields, or memory body were recorded. The only remaining memory gate is persistence after a manual Render restart.
