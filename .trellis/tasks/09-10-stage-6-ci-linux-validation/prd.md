...
- 初次基线为 `a234e599`；兼容依赖修复后的恢复以 `f9c1133f` 为新基线。每次从固定提交导出已跟踪文件，使用容器内独立副本、干净 npm 和浏览器缓存；执行实际 YAML 中原样提取的全部 run 步骤，不混用不同锁文件的成功结果。
...
- [ ] 本轮主仓库源码、依赖、原构建与保护快照未变；全部任务容器移除，既有资源保留。
