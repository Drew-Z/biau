...
- 初次基线为 `a234e599`；兼容依赖修复后使用 `f9c1133f`，官方源地址修复后使用 `d06b5ace`。每次从固定提交导出已跟踪文件，使用容器内独立副本、干净 npm 和浏览器缓存；执行实际 YAML 中原样提取的全部 run 步骤，不混用不同锁文件的成功结果。
...
- [ ] 新基线的干净 `npm ci`、lint、build、四项本地合同和性能预算实际通过；旧基线结果单独保留。
