...
- [ ] 保存 `loop.md`、`assessment.md` 和 `task.json.meta.loop`，核对工作流入口一致。
- [ ] 第一轮：审查并交付 7 个历史 UI 修改文件，复用本会话已通过且代码未改变的 UI-012 完整验证。
- [ ] 返回父任务，重新评估并完成博客/项目列表状态的下一项独立子任务。
- [ ] 用首轮实际执行结果核验协议，再创建每 15 分钟的同会话 heartbeat 并将标识回写。
