...
建立一个长期主任务，用来持续发现、拆分、排序和推进 BIAU Port / 泊岸项目的后续优化工作。主任务本身不直接承载所有实现细节，而是作为任务编排层，把公开助手、项目展示、可靠性观察、ERP/Legal/Pet/AI 日报等独立可验证工作纳入同一条持续改进主线。
...
2. Select the highest-value unblocked child task.
3. If the child is planning-only, finish PRD/design/implement and ask for start approval unless the user has already authorized implementation.
4. If in progress, implement, validate, record spec learnings, commit, and push when appropriate.
5. Add newly discovered work to the task tree or candidate backlog.
6. Update `manual-actions.md`.
