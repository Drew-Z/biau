...
- Add Cloudflare Pages Functions that relay only OpenAI-compatible Responses requests to separate fixed approved primary and fallback upstream channels.
...
- [x] Primary and fallback relay routes use separate fixed upstream bindings while sharing only the server-only auth token and bounded model allowlist.
...
- Relay diagnostic revision `87210661` is live on Cloudflare Pages and Render. Deterministic checks, both health boundaries, and the unauthenticated relay `401` boundary pass.
- A user-approved request made after warming the free Render service returned HTTP `200` with `degraded/fallback` after three bounded attempts. The low-sensitive recovery event classified it as `provider_unavailable`, excluding relay transport, content-contract, and size failures; the temporary session was deleted successfully. The final acceptance criterion remains open and another live request requires fresh approval.
