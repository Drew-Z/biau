...
- Public repository license follow-up is complete with Apache-2.0 for `biau`, `legal-rag`, `gamer`, `xunqiu`, `blog`, `space-impact`, and `spacewar-II`. `legal-rag` now has remote `main` as its default branch. `gamer` keeps the broader `cursor-windows-migration` work in draft PR `Drew-Z/gamer#2` instead of merging it directly because it includes implementation changes beyond README/showcase packaging.
