...
- Empty composer placeholders must fit inside the textarea in both languages. A contained control rectangle does not prove its internal text is visible: after mobile fullscreen reaches the viewport width and fonts settle, assert the empty input has `scrollHeight <= clientHeight + 1`. The existing `max-width: 360px` rule reserves three lines at the unchanged 1.45 line-height plus vertical padding and borders. Check both sides of this breakpoint, short viewports, long-draft internal scrolling and reachable 44px actions; retain the existing copy, font size and bounded input behavior.
