...
- Good: first verification finds contradicted/insufficient prose, one evidence-aware replacement removes or narrows it, one verifier recheck passes, and deterministic validation produces the only eligible result.
- Base: the replacement still lacks official evidence or independent trend domains; deterministic validation rejects it and the runner does not start a second repair cycle.
- Bad: send claims without their evidence roles/domains, retry quality repair until the model agrees, or turn an `official-evidence-required` finding into editor-only review.
