...
后续审查确认，固定要求每期至少出现两条新鲜厂商官方文章并不适合日报业务：The News API、GDELT、Hacker News 与 HotDaily 已经可以发现实时事件，系统也会独立抓取原文、校验日期并保留引用证据。官方 RSS 应作为高权重第一方证据与高风险声明校验来源，而不是厂商没有发文时阻止整期成稿的数量门槛。

新策略继续保留至少五个合格事件、三个独立域名、原文 `READY`、时间窗、AI 相关性和完整引用；发布、日期、价格、API 与可用性声明仍必须绑定官方证据，跨事件趋势必须由至少两个独立域名支撑。Apple、Google Cloud 与 IBM 的 hold 状态不变，它们是后续覆盖补充，不再是首期日报的阻塞项。
