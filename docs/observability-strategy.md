...
- 使用 `npm.cmd run reliability:check` 做跨项目发布前或发布后检查；它默认只写受控临时目录。确认需要更新公开快照时再运行 `npm.cmd run reliability:publish`；需要单独巡检核心路由时继续运行 `npm.cmd run site:monitor`。
...
- `biau_ai_daily_latest_run_end_to_end_lag_available` / `biau_ai_daily_latest_run_freshness_available`
...
- `biau_ai_daily_failure_signals{category="config|provider|evidence|quality|infrastructure|stale-content"}`
- `biau_ai_daily_issues_total`
- `biau_ai_daily_public_flash_items_active` / `biau_ai_daily_public_flash_age_seconds`
- `biau_ai_daily_public_flash_available`
- `biau_ai_daily_retention_due_total` / `biau_ai_daily_alerts_total`

受 Studio token 保护的 `GET /studio/api/ai-daily/operations` 返回同一份低敏 JSON diagnostics。它不会返回 id、URL、标题、正文、provider 身份、错误原文或凭据；只有 `METRICS_ENABLED=true` 且 `AI_DAILY_OPERATIONS_METRICS_ENABLED=true` 时 `/metrics` 才追加 snapshot，查询失败只输出 `biau_ai_daily_operations_snapshot_up 0`。年龄和 lag 指标必须与对应 `*_available` gauge 一起消费，避免把缺失值的兼容性 `0` 当作新鲜数据。

AI Daily failure signal 是当前状态与最近 24 小时持久化信号的低敏投影，不是唯一事故数。仓库中的 `observability/ai-daily-grafana-dashboard.json` 和 `observability/ai-daily-prometheus-alerts.yml` 可作为导入模板；看板还覆盖来源健康、队列、运行新鲜度、provider role、issue 生命周期、公开内容年龄和 retention due。`npm.cmd run ai-daily:observability-contract-check` 会离线检查这些面板表达式、六类故障规则、独立的 snapshot unavailable 告警、severity 和敏感字段边界；真实 datasource、scrape 与通知渠道仍由平台人工配置。
