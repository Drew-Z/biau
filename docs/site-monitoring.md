...
| Search Console / Webmaster | 搜索曝光、点击、查询词、收录和 sitemap 状态 | 全站真实访问人数、站内点击、接口健康 | 需要 SEO 数据时推荐配置，不阻塞站点可用性 |
...
`site:monitor --check-external` 只能发现服务端 HTML 里的外部资源；React
数据里配置的首页卡片、项目详情按钮、游戏试玩、GitHub、API health、展示页
等链接由显式链接清单检查覆盖：
