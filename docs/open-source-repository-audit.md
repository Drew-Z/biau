...
| `xunqiu` / `xunqiu-backend-modern` | Showcase and backend READMEs plus Chinese entry points are packaged. | Static showcase has no build step; backend Maven build/test and jar smoke were verified. | Cloudflare Pages showcase and Render-style backend deployment are documented with placeholders. | Showcase file/link checks; backend Maven test/package, jar smoke, Docker build, sensitive scan. | Backend visibility, APK release/SHA-256/signing policy, production auth/rate-limit/CORS. |
