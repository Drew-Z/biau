...
| `D:\workspace4Cursor\blog-semi` | First slice complete | Root README rewritten as an open-source entry point covering the main site, owner-only LangGraph BIAU Operator, Studio, AI Daily, four-service deployment boundary, RAG Orchestrator, checks, and public-safe rules. Follow-up removed stale inline screenshots, added `README.zh-CN.md`, tracked screenshot assets in `docs/showcase-assets.md`, moved local-only agent/Trellis history out of Git tracking, updated GitHub description/homepage/topics on 2026-07-09, and added Apache-2.0 license. On 2026-07-10 the homepage, projects, and blog README preview screenshots were regenerated from current local routes and re-embedded in both README languages. | README link/image/sensitive-shape checks; `npm.cmd run docs:manual-gates-check`; `npm.cmd run docs:deployment-check`; `git diff --check` only reported LF/CRLF normalization warnings. Hidden-tooling audit kept `.github/workflows/`, `.agents/skills/`, `.codex/`, `.claude/`, reusable Trellis specs/scripts/current tasks, and removed `.agent-work/`, `.trellis/workspace/`, `.trellis/tasks/archive/` from public tracking. 2026-07-10 screenshot refresh: local Vite route capture with the first-entry intro skipped, followed by `npm.cmd run images:optimize -- --force`. | Optional deploy buttons; production provider secrets stay platform-only; review GitHub social preview image manually in the GitHub UI if a repository-level Open Graph image is desired. |
...
Reviewed on 2026-07-10 after the repository packaging slices and main-site sync checks.

| Repository | README status | Quick-start status | Deployment status | Tests / checks recorded | Remaining manual gates |
| --- | --- | --- | --- | --- | --- |
| `blog-semi` / `Drew-Z/biau` | English and Simplified Chinese README entry points are current, with refreshed homepage/projects/blog screenshots. | Documented Node/Vite workflow; local build and docs checks pass. | Four-service Render boundary and static site deployment docs are aligned. | `assistant:index`, `project-details:check`, `public-links:check`, `docs:manual-gates-check`, `docs:deployment-check`, `lint`, `build`. | Optional GitHub social preview image; production secrets and provider settings stay platform-side. |
| `legal-rag` | Open-source README and Chinese README are packaged; stale screenshots removed from the GitHub landing page. | Workspace commands are documented and previously verified. | Docker Compose and Render/Supabase-style deployment docs are public-safe. | Typecheck, API unit/validate/evaluate checks, build, Compose config, follow-up diff checks. | Public demo credentials, live model/pgvector validation, refreshed screenshots. |
| `erp` | README and Chinese README are packaged for future public release, but the repo remains private by decision. | Local test/build and Docker Compose config were verified. | Deployment instructions avoid real SFTP/Ozon/private values. | Python compile, Docker Compose config, package script check, test, build. | Keep private for now; choose license and demo account policy before public release. |
| `xunqiu` / `xunqiu-backend-modern` | Showcase and backend READMEs plus Chinese entry points are packaged; the public showcase contains no APK artifact. | Static showcase has no build step; backend Maven build/test and jar smoke were verified. | Cloudflare Pages showcase and Render-style backend deployment are documented with placeholders. | Showcase file/link/release-gate checks; backend Maven test/package, jar smoke, Docker build, sensitive scan. | Backend visibility, future APK release/SHA-256/signing approval, production auth/rate-limit/CORS. |
| `pet` / `gamer` showcase | Static Pet showcase README and Chinese entry point are packaged; broader Pet repos remain gated by dirty worktrees. | Static showcase file checks pass. | Static hosting and APK download gate are documented. | Showcase href/file checks and focused sensitive scan. | Resolve dirty Pet worktrees; approve APK, signing, release notes, screenshots. |
| `game` / Playlab / Godot repos | Playlab, Space War, and Spacewar II README surfaces are packaged with Chinese entry points where applicable. | Astro and Godot smoke/capture workflows were verified in their slices. | Cloudflare Pages and separate playable export hosting are documented. | Playlab verify/content audit, Godot smoke/capture, diff checks, focused sensitive scans. | Review public screenshots/videos/playable links and GitHub social preview images. |

Conclusion: the packaging task has reached a public-safe first pass for the selected repositories. The remaining items are mostly account-side, credential-side, release-signing, or production-validation gates rather than README structure blockers.
...
- Main `Drew-Z/biau` README screenshots were regenerated and embedded on 2026-07-10. Review GitHub repository social preview images in the GitHub UI if custom Open Graph cards are desired; they are account-side settings.
...
- Showcase `README.md`, `index.html`, `docs.html`, `_headers`, `.gitignore`, `docs/technical/*`, and `assets/*`; the previously public `downloads/latest-xunqiu64.apk` was removed after its matching local archive was verified.
...
- Future APK publication still requires SHA-256, release notes, official signing policy, scan/regression evidence, rollback notes, and maintainer approval; no public APK remains in the showcase repository.
