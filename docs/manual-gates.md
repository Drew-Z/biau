...
2026-08-16 已完成 CPA 侧的零调用切换交付：Public API 的当前主模型为精确 `free5/DeepSeek-V4-Flash`，生产 `PUBLIC_ASSISTANT_MODEL_MAX_ATTEMPTS=1`，真实 CPA base/key 仍只放 Render server-only 环境。AI Daily proposal `ai-daily-cpa-deepseek-v4-flash-static-v1` 的 hash 为 `508e23df7a6b53f7aee74fee6845fc5686f2b5988208e1a745be3868cefbb263`，三角色统一 `providerRef=cpa-gateway`、`failureDomainRef=cpa-free5`；站点所有者已明确批准并生成 bundle hash `4fa08db8374bef1e8bdc485ad626a69b3765da6efdbda6a8f7253aaa24a70248`，命令确认 `modelCalls=0`。随后已在 Render 交付 runtime、Secret File 和 hash：Public API 部署 `dep-da0sctlbedkc73bgnra0`（`c4a13197`）live，`/health=200` 且 `modelConfigured=true`；Studio 部署 `dep-da0sqpe1egvs739g6oq0`（`fc08c2a1`）live，启动日志确认 `networkCalls=0`、1 channel、3 candidates、1 failure domain，bundle hash 完全匹配。Studio `/health=200`、未授权 workspace `401 missing-studio-token`、public Feed `404`，部署窗口无 error-level 日志；AI Daily 未执行真实模型请求、真实 Edition、Studio 审核或公开发布。
...
站点所有者随后对已交付的 v6 bundle 单独批准并执行一次真实 Edition。受保护入口只提交一次，Run `cmsxar81600004bal6qbas8wr` 以 `COMPLETED_WITH_GAPS` 结束；两次 extractor、两次 composer 和两次 verifier 调用全部成功，证明初稿、首轮验证、单次质量修复和复核均完整执行。Revision 14 `cmsxauh6s000c4bal87hkuhip` 仍为 `REJECTED` / `DISCARDED`，剩余四条 critical：`composition:subtitle`、`composition:introduction` 与 `event:evt-openai-bedrock:why-it-matters` 均为 `scope_inflation`，claim `grok-bot-launch` 为 `official-evidence-required`。没有创建 draft、review、Publish Export、内容部署或公开发布。

真实窗口已安全关闭：恢复部署 `dep-da1h7mnqj5pc73d21le0` 为 `live`，production generation 与 stage diagnostics 均为 `disabled`，`PENDING` / `LEASED` / `RETRY_WAIT`、backlog、活动阶段和 expired lease 均为 0，Public Feed 仍为 `404`，关闭部署后的 error-level 日志为 0。
