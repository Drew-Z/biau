...
仓库提供 `observability/ai-daily-grafana-dashboard.json` 和 `observability/ai-daily-prometheus-alerts.yml`，并由 `npm.cmd run ai-daily:observability-contract-check` 离线验证六类故障 panel/rule、独立的 operations snapshot unavailable 告警、固定 severity、套件注册和敏感字段禁入。模板不包含真实 datasource、scrape URL 或通知目标；导入生产 Grafana/Prometheus/ARMS 和配置通知路由仍需人工完成。
