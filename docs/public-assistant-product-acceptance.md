...
知航 BIAU Beacon 当前为 **工程就绪，生产检索与 CPA 生成链路已通过一次真实业务验收，产品可用**。确定性合同已覆盖公开 API、会话版本、分支、浏览器状态、持久化、恢复、安全、取消和降级语义；生产侧直接消费单租户 CPA Responses 网关，并将最终回答生成上限固定为一次。2026-08-16 的同源 SSE 业务任务得到非降级回答和 verified 站内引用；结合此前已通过的刷新持久化、断网恢复与 390px 移动端观察，产品级门禁已关闭。该结论只证明一个受控业务闭环，不等于长期 SLA，默认 synthetic 仍不调用模型。
...
| 直接 Render 运维入口 | 执行一次站内产品比较问题；Render 网关返回 `504`，没有回答、引用或可验收的持久化结果 |
| 服务端证据 | 对应窗口没有匹配的 Render 应用/请求日志；临时会话删除返回 `404`，无法证明会话已创建或模型上游已完成调用 |
| 真实访客 SSE | 改用主站同源 `/api/chat/public/stream` 执行一条新的画帆状态业务问题；返回完整 `ready -> progress -> result -> done`，终态 `answered` / `model`，一次生成尝试，耗时约 14 秒 |
| 引用与持久化 | 返回 1 条 `verified` 站内引用，`/projects/canvas` 核验为 HTTP `200`；响应包含完整 conversation identity，临时会话删除成功且删除后读取为 `404` |
| 结论 | 本次非降级回答、引用和会话边界通过；结合此前已通过的刷新持久化、失败恢复与 390px 移动端观察，产品级门禁关闭 |
...
1. 同源 SSE 的非降级模型回答、verified 引用与临时会话清理已经通过。
2. 此前通过的浏览器断网恢复、桌面刷新持久化和 390px 手机无溢出继续作为同一产品门禁的组合证据。
3. 公开状态可以更新为“产品可用”，但不能把单次成功表述为长期 SLA、自动发布或渠道稳定性保证。
4. 后续只有真实业务回归可以再次调用模型；禁止 ping、doctor、空 prompt、catalog probe、逐模型测活或默认 synthetic 模型调用。
