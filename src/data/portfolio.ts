...
export const projects: Project[] = [
  ...
  },
  {
      ...
      quality: [
        {
          ...
          items: [
            ...
            '`assistant:hybrid-contract`、`assistant:rag-smoke`、`assistant:rag-pgvector-schema-check` 和 `assistant:public-sync-check` 守住 pgvector schema、关键词/向量/实体关系混合检索、reranker/fallback 元数据与 public-only 同步。',
    ...
    assistantContext: [
      ...
      '公开助手没有登录、成员、私有知识、长期记忆或写入工具；Tavily 只提供发现线索，最终 citation 只能来自 SSRF-safe 抓取的公开原网页或 public-only RAG 证据。',
      'RAG Orchestrator 生产存储使用 Supabase pgvector，组合 4096 维精确余弦检索、关键词召回和轻量实体关系扩展；本地与可选 Qdrant 适配器只用于确定性测试或回滚兼容。',
      ...
      '当前边界是静态公开数据驱动、匿名只读研究、人工审核发布和生产配置 gate；Operator/internal-RAG 退役已经完成，后续人工队列聚焦 Studio 被退回草稿修订、证据完整版本审核、Publish Export 与各关联项目的受控发布 gate。',
    ],
  },
  {
    id: 'canvas',
