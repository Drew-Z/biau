...
export const reliabilityProjects: ReliabilityProject[] = [
  {
    ...
    checks: [
      ...
      },
      {
        ...
        status: 'online',
        description: '同域 health 与一次获批的真实 SSE 业务任务已通过；默认 synthetic 仍只验证 `/api/health`，不会自动触发模型调用。',
        evidence: '真实访客链路完成 `ready -> progress -> result -> done`，终态为 answered/model、一次生成尝试，并返回 1 条可访问的 verified 站内引用；此前刷新持久化、断网恢复和 390px 移动端观察已通过。`main-site:synthetic` 默认继续禁用 live chat。',
      ...
      },
      {
        ...
        status: 'online',
        description: '检查外部 RAG Orchestrator `/health`、Supabase pgvector readiness、hybrid retrieval 和低敏公开同步结果。',
        evidence:
          '生产 RAG Orchestrator 已完成低敏验收：store=supabase-pgvector，4096 维向量与关键词/实体关系混合检索可用，事务式公开知识同步和 citation 投影通过受控业务流验证。真实 RAG token、数据库 URL、embedding endpoint 和文档正文不进入公开状态。',
        cadence: '部署后 + 每日',
    ...
    nextActions: [
      ...
      '把生产 RAG Orchestrator `/health`、pgvector readiness 与公开知识 sync 的低敏摘要纳入定时 synthetic 或内部监控。',
    ],
  },
  {
    id: 'legal-rag',
    ...
    checks: [
      {
        ...
        label: '邀请制工作台入口',
        status: 'unchecked',
        description: '检查 Chatus 根入口是否响应并进入登录或受限访客工作区。',
        evidence: '由 L0 入口检测自动合并；入口可达不代表成员账号或完整能力公开。',
        cadence: '每次生成状态页',
        ownerHint: '公开入口',
        relatedTargetId: 'chatus-entry',
      },
      {
        id: 'chatus-session-boundary',
        layer: 'synthetic',
        label: 'Session 与受限访客边界',
        status: 'planned',
        description: '使用专用测试身份验证成员 session、访客能力收窄和撤销后的旧会话行为。',
        evidence: '主站不保存或运行访问材料；credentialed 检查只能留在 Chatus 自身的受控发布流程。',
        cadence: '鉴权或 guest 策略变更后',
        ownerHint: 'Chatus CI',
      },
      {
        id: 'chatus-stream-recovery',
        layer: 'synthetic',
        label: '可恢复流与首输出前回退',
        status: 'planned',
        description: '验证空流、畸形事件流、候选忙和首个可见输出后的禁止回退语义。',
        evidence: '仓库测试覆盖 provider 协调、流协议、工具输出和回退边界；主站不主动触发模型请求。',
        cadence: '流协议或 provider router 变更后',
        ownerHint: 'Chatus protocol tests',
      },
      {
        id: 'chatus-release-gate',
        layer: 'observability',
        label: '精确版本发布门禁',
        status: 'planned',
        description: '生产发布先完成前端、测试、类型检查和部署 dry-run，再按精确提交版本执行低敏 smoke。',
        evidence: '生产变更由 Chatus 独立 CI 所有；主站只展示公开入口与低敏技术边界。',
        cadence: '每次生产发布',
        ownerHint: 'Chatus deploy workflow',
      },
    ],
    gates: [
      '不得把真实访问码、成员身份、provider 配置、托管密钥或运营指标复制到主站状态数据。',
      '不得用主动 completion 测活代替真实任务脱敏遥测；模型调用只在 Chatus 自身获得批准的测试边界内发生。',
    ],
    nextActions: [
      '持续检查邀请制入口的 HTTP 可达性与登录页面渲染。',
      '鉴权、访客策略或流协议变更后，在 Chatus 独立 CI 中运行 credentialed synthetic，不在主站保存凭据。',
    ],
  },
  {
    id: 'anchor-learning',
    ...
    checks: [
      {
        id: 'anchor-learning-entry',
        layer: 'entry',
        label: '浏览器 Demo HTTP 入口',
        status: 'unchecked',
        description: '检查公开 `/app/` URL 是否返回可接受的 HTTP 响应。',
        evidence: '由 L0 入口检测自动合并；它不验证官网内容、规范化重定向、答题交互或完整 Flutter 产品。',
        cadence: '每次生成状态页',
        ownerHint: '公开入口',
        relatedTargetId: 'anchor-learning-entry',
      },
      {
        id: 'anchor-learning-demo-flow',
        layer: 'synthetic',
        label: '12 题浏览器学习流程',
        status: 'online',
        description: '验证三套数据、单选/多选/判断、解释、locator、来源摘录、导师提示、进度恢复与重置。',
        evidence: '2026-07-27 生产 Playwright 在桌面、平板和手机视口完成 8 项流程与视觉检查。',
        cadence: '每次 Web 发布',
        ownerHint: 'Anchor Playwright',
      },
      {
        id: 'anchor-learning-no-external-requests',
        layer: 'synthetic',
        label: 'Demo 零外部数据请求',
        status: 'online',
        description: '浏览器答题流程不得访问后端、上传服务、分析服务或 AI provider。',
        evidence: '生产浏览器检查按目标 origin 捕获所有请求；Pages 响应使用 `no-transform` 阻止边缘分析脚本注入。',
        cadence: '每次 Web 发布或 Cloudflare 配置变更后',
        ownerHint: 'Anchor request audit',
      },
      {
        id: 'anchor-learning-flutter-release',
        layer: 'observability',
        label: 'Flutter Private Alpha 发布证据',
        status: 'planned',
        description: 'Android Private Alpha 需要独立验证安装、升级、备份恢复、数据库迁移和正式构建。',
        evidence: '浏览器 Demo 的通过结果不能替代 Flutter 客户端发布门禁。',
        cadence: '每次 Android Alpha 发布',
        ownerHint: 'Anchor mobile release',
      },
    ],
    gates: [
      '浏览器 Demo 只能宣称打包数据与静态交互，不宣称实时 AI、文件导入、云同步或 Flutter Web parity。',
      '任何真实模型验收都需要单独批准，当前主站和 Demo 回归不得发起 provider 调用。',
    ],
    nextActions: [
      '把生产 `/`、`/app/`、`/app/index.html` 和无外部请求检查保持在发布回归中。',
      'Flutter 客户端按 Android Private Alpha 清单继续补齐安装、升级和本地数据迁移证据。',
    ],
  },
  {
    id: 'ozon-erp',
