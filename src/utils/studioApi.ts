...
export function explainStudioApiError(status: number, errorCode: string) {
  ...
  if (errorCode === 'invalid-ai-daily-stage-diagnostic') return '阶段诊断请求缺少有效角色、当前 Edition 版本或明确确认。'
  if (errorCode === 'ai-daily-stage-diagnostics-disabled') return '单阶段诊断当前关闭；仅在短时调试窗口启用 AI_DAILY_STAGE_DIAGNOSTICS_ENABLED。'
  if (errorCode === 'ai-daily-stage-diagnostics-production-active') return '生产生成或业务评测仍在运行；关闭后才能执行单阶段诊断。'
  if (errorCode === 'ai-daily-stage-diagnostics-configuration-invalid') return '阶段诊断无法加载当前模型运行时、审批文件或审批哈希。'
  if (errorCode === 'ai-daily-stage-diagnostics-busy') return '已有一个阶段诊断正在运行；当前渠道不会重复调用。'
  if (errorCode === 'ai-daily-stage-diagnostics-issue-not-found') return '没有找到要诊断的 AI Daily Edition。'
  if (errorCode === 'ai-daily-stage-diagnostics-issue-version-conflict') return '当前 Edition 已更新；请刷新工作区后重新确认诊断。'
  if (errorCode === 'ai-daily-stage-diagnostics-evidence-not-ready') return '当前 Edition 的真实证据包未通过授权或完整性检查。'
  if (errorCode === 'ai-daily-stage-diagnostics-revision-required') return 'Composer 或 Verifier 诊断需要当前 Edition 的最新生成 revision。'
  if (errorCode === 'ai-daily-stage-diagnostics-revision-invalid') return '最新生成 revision 无法通过当前 claims/composition 合同校验。'
